...
ibrx <- 
  ...
  filter(ibrx == 1,
         !(empresa %in% c('BBDC3',
                          ...
                          'USIM3',
                          'VALE5'))) %>% 
  mutate(empresa2 = recode(empresa, 
                           '0927042D' = 'CCXC3',
                           'SUZB5'    = 'FIBR3',
                           'GOAU4'    = 'GGBR3',
                           'RUMO3'    = 'RAIL3',
...
fit1 <- 
  ibrx %>% 
  lme(retorno_anual ~ factor(ceo_chair) + ano_adj, random = ~ 1 | empresa, data=., method = "ML")
summary(fit1)

fit2 <- 
  ibrx %>% 
  lme(retorno_anual ~ factor(ceo_chair) + ano_adj + I(ano_adj^2), random = ~ 1 | empresa, data=., method = "ML")
summary(fit2)
...
fit.final <- 
  ibrx %>% 
  lme(retorno_anual ~ factor(ceo_chair) + ano_adj + I(ano_adj^2), random = ~ 1 | empresa, data=.)
summary(fit.final)
